        ...
        <>
          <ul className="divide-y divide-gray-200">
            {visible.map((it, i) => {
              ...
              const stripe = i % 2 === 0 ? 'bg-transparent' : 'bg-gray-50'
              const row = (
              ...
              return (
                <li key={it.id} className={cn(stripe, 'transition-colors hover:bg-gray-50')}>
                  {it.href ? (
          ...
          <footer className="flex items-center justify-between border-t border-gray-200 px-5 py-3 text-xs">
            <span className="text-gray-400 tabular-nums">
              Showing {visible.length} of {totalLoaded}
              {totalLoaded === 50 ? '+' : ''}
            </span>
            <div className="flex items-center gap-1">
              <span className="mr-1 text-gray-400">Show</span>
                ...
                return (
                  <button
                    ...
                    className={cn(
                      ...
                      disabled && 'cursor-not-allowed opacity-40 hover:bg-transparent hover:text-gray-500',
                    )}
                  >
                    {size}
                  </button>
