...
const DOODLE_BG_CLASSES =
  "bg-gray-100 bg-[url('/inbox-doodle.svg')] bg-repeat";
...
export function MessageThread({
  ...
  const handleSendMedia = useCallback(
    async (
      ...
      const optimisticMsg: Message = {
        ...
        content_text: media.type === "document" ? media.filename : undefined,
        media_url: media.url,
        status: "sending",
        created_at: new Date().toISOString(),
        reply_to_message_id: replyToId,
  ...
  return (
    <div className={cn("flex flex-1 flex-col", DOODLE_BG_CLASSES)}>
      {/* Header — solid bg-gray-50 sits on top of the doodle so the
          name/avatar/dropdowns stay legible. */}
      <div className="flex items-center justify-between gap-2 border-b border-gray-200 bg-gray-50 px-3 py-3 sm:px-4">
        <div className="flex min-w-0 items-center gap-2 sm:gap-3">
          ...
          {onBack && (
            <button
              ...
              className="flex h-9 w-9 flex-shrink-0 items-center justify-center rounded-md text-gray-700 hover:bg-gray-50 hover:text-gray-900 lg:hidden"
            >
              <ArrowLeft className="h-5 w-5" />
            </button>
          )}
          <div className="flex h-9 w-9 flex-shrink-0 items-center justify-center rounded-full bg-gray-100 text-sm font-medium text-gray-900">
            {displayName.charAt(0).toUpperCase()}
          </div>
          <div className="min-w-0">
            <h2 className="truncate text-sm font-semibold text-gray-900">{displayName}</h2>
          ...
          <Badge
            variant="outline"
            className={cn(
              "ml-1 hidden gap-1 border-gray-200 text-[10px] sm:inline-flex sm:ml-2",
              sessionInfo.expired ? "text-red-600" : "text-primary"
            )}
          >
        ...
        <div className="flex items-center gap-2">
          ...
          {onRefresh && (
            <button
              ...
              title="Refresh"
              className={cn(
                "inline-flex h-7 w-7 items-center justify-center rounded-md text-gray-500 transition-colors hover:bg-gray-50 hover:text-gray-900 disabled:opacity-60",
              )}
            >
              <RefreshCw
          ...
          <DropdownMenu>
            <DropdownMenuTrigger className={cn(
                  "inline-flex items-center justify-center h-7 gap-1 px-2 text-xs rounded-md hover:bg-gray-50",
                  currentStatus?.color ?? "text-gray-500"
                )}>
          ...
          <DropdownMenu>
            <DropdownMenuTrigger
              className={cn(
                "inline-flex items-center justify-center h-7 gap-1 px-2 text-xs rounded-md hover:bg-gray-50",
                assignedAgentId ? "text-primary" : "text-gray-500"
              )}
            >
            ...
            {messageGroups.map((group) => (
              <div key={group.date}>
                ...
                <div className="mb-4 flex items-center justify-center">
                  <span className="rounded-full bg-white px-3 py-1 text-[10px] font-medium text-gray-500">
