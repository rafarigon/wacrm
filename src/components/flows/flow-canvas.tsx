    ...
    >
      {hasTarget && (
        <Handle
          ...
          className="!h-2.5 !w-2.5 !border-gray-300 !bg-gray-100"
        />
      )}

      <div className="flex items-center gap-2">
        <Icon className={cn("h-3.5 w-3.5 shrink-0", meta.color)} />
        <span className="truncate text-[11px] font-medium uppercase tracking-wide text-gray-500">
          {meta.label}
        </span>
        {isEntry && (
          <span className="ml-auto rounded bg-emerald-500/15 px-1.5 py-0.5 text-[9px] font-semibold uppercase tracking-wider text-emerald-700">
            Entry
          </span>
        )}
      </div>
      ...
      {isMultiSlot && (
        <div className="mt-2 flex flex-col gap-1 border-t border-gray-200 pt-2">
          {slots.map((slot) => (
            <div
              key={slot.id}
              className="relative flex items-center justify-between gap-2 rounded px-1 py-0.5 text-[11px] text-gray-700"
            >
              ...
              <Handle
                ...
                className="!relative !right-auto !top-auto !h-2.5 !w-2.5 !translate-x-[12px] !transform-none !border-gray-300 !bg-gray-100"
              />
            </div>
          ))}
        </div>
...
function FlowCanvasInner() {
  ...
  if (rfNodes.length === 0) {
    return (
      <div className="flex h-[60vh] flex-col items-center justify-center gap-3 rounded-lg border border-dashed border-gray-200 bg-gray-100 text-sm text-gray-400">
        <p>No nodes yet.</p>
        <CanvasAddNodeButton />
      </div>
    );
    ...
    <>
      <div className="h-[70vh] w-full overflow-hidden rounded-lg border border-gray-200 bg-gray-100">
        ...
        >
          ...
          <Controls
            className="!border-gray-200 !bg-gray-50 [&_button]:!border-gray-200 [&_button]:!bg-gray-50 [&_button:hover]:!bg-white"
...
function NodeEditSheet({
  ...
  return (
    <Sheet open={open} onOpenChange={(v) => !v && onClose()}>
      <SheetContent
        side="right"
        className="flex w-full flex-col gap-0 border-l border-gray-200 bg-gray-100 p-0 sm:max-w-md"
      >
        <SheetHeader className="border-b border-gray-200 px-5 py-4">
          <SheetTitle className="flex items-center gap-2 text-gray-900">
            ...
            {isEntry && (
              <span className="rounded bg-emerald-500/15 px-1.5 py-0.5 text-[10px] font-semibold uppercase tracking-wider text-emerald-700">
                Entry
              </span>
            )}
          </SheetTitle>
        ...
        <SheetFooter className="border-t border-gray-200 px-5 py-3 sm:flex-row sm:justify-between">
          {!isEntry ? (
...
function CanvasAddNodeButton() {
  ...
  return (
    <DropdownMenu>
      <DropdownMenuTrigger
        className="inline-flex items-center gap-1.5 rounded-md border border-gray-200 bg-gray-50 px-3 py-1.5 text-xs font-medium text-gray-800 shadow-lg transition-colors hover:bg-gray-50"
        aria-label="Add node"
      >
