...
export function Step4ScheduleSend({
  ...
  return (
    <div className="space-y-6">
      ...
      {/* Broadcast Name */}
      <div>
        <label className="mb-1.5 block text-sm font-medium text-gray-900">Broadcast Name</label>
        <Input
          value={name}
      ...
      <div className="rounded-xl border border-gray-200 bg-gray-50 p-4 space-y-3">
        ...
        <div className="grid grid-cols-2 gap-3 text-sm">
          ...
          <div>
            ...
            <p className="text-gray-900">{audienceLabel}</p>
          </div>
          <div>
            ...
            <div className="flex items-center gap-1.5">
              {loadingReach ? (
                ...
                <>
                  ...
                  <p className="font-medium text-gray-900">{estimatedReach.toLocaleString()}</p>
                </>
              )}
            </div>
          </div>
          <div>
            ...
            <p className="text-gray-900">{template.language ?? 'en_US'}</p>
          </div>
        </div>
      </div>
        ...
        <div className="rounded-xl border border-primary/20 bg-primary/5 p-4">
          <div className="mb-2 flex items-center justify-between">
            <div className="flex items-center gap-2">
              ...
              <p className="text-sm font-medium text-gray-900">Sending broadcast...</p>
            </div>
      ...
      <div className="flex flex-wrap items-center justify-between gap-2 border-t border-gray-200 pt-4">
        ...
        <div className="flex items-center gap-2">
          ...
          <DialogContent className="border-gray-200 bg-gray-50 sm:max-w-md">
            <DialogHeader>
              <DialogTitle className="text-gray-900">Confirm Broadcast</DialogTitle>
              <DialogDescription className="text-gray-500">
                You are about to send this broadcast to{' '}
                <span className="font-medium text-gray-900">{estimatedReach.toLocaleString()}</span>{' '}
                contacts using the{' '}
                <span className="font-medium text-gray-900">{template.name}</span> template.
