...
export function Step1ChooseTemplate({ selectedTemplate, onSelect, onNext, onBack }: Step1Props) {
  ...
  return (
    <div className="space-y-6">
      ...
      {templates.length === 0 ? (
        <div className="flex h-48 flex-col items-center justify-center rounded-xl border border-gray-200 bg-gray-50">
          <FileText className="mb-2 h-8 w-8 text-gray-400" />
          <p className="text-sm text-gray-500">No templates available.</p>
          <p className="mt-1 text-xs text-gray-400">Create a template in Settings first.</p>
        </div>
      ) : (
          ...
          {templates.map((template) => {
            ...
            return (
              ...
              >
                ...
                <p className="line-clamp-3 text-xs text-gray-500">{template.body_text}</p>
