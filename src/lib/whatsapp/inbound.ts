/**
 * Provider-agnostic inbound WhatsApp pipeline.
 *
 * Everything below was extracted verbatim from
 * src/app/api/whatsapp/webhook/route.ts so that a second provider
 * (WAHA — see src/app/api/whatsapp/waha-webhook/route.ts) can feed the
 * exact same contact / conversation / message / flows / automations
 * pipeline without duplicating business logic. The Meta webhook route
 * keeps signature verification + JSON parsing and delegates here.
 *
 * Messages from non-Meta providers are translated into the Meta
 * `WhatsAppMessage` shape before reaching processMessage. Media from
 * those providers arrives already resolved to a URL (no Meta media-id
 * verification round-trip), carried on the optional `direct_media`
 * field.
 */
...
export interface WhatsAppMessage {
  ...
  context?: { id: string }
  /**
   * Non-Meta providers only (WAHA). Media already resolved to a fetchable
   * URL at translation time — parseMessageContent uses it directly and
   * skips the Meta media-id verification round-trip. `message.type` must
   * still be set to image/video/audio/document so the messages.content_type
   * CHECK constraint mapping applies unchanged.
   */
  direct_media?: {
    url: string
    mime_type: string
...
async function parseMessageContent(
  ...
  // Non-Meta providers (WAHA) resolve media to a URL at translation time —
  // no Meta media-id verification applies. Checked before the switch so a
  // WAHA image/video/document/audio message never hits verifyAndBuildUrl.
  if (message.direct_media) {
    return {
      ...empty,
      contentText:
        message.direct_media.caption || message.direct_media.filename || null,
      mediaUrl: message.direct_media.url,
      mediaType: message.direct_media.mime_type,
    }
  }
